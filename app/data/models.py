...
class CustomerPhone(Model):
    ...
    entity_id = Column(UUIDType(binary=False), nullable=False)
    entity_type = Column(entity_types)
    phone_number = Column(String)
...
class SubOrder(Model):
    ...
    order_status = Column(ENUM('pending', 'ongoing', 'done', name='sub_order_status'))
...
class ExtraOrder(Model):
    ...
    courier_price = Column(Float)
    country_code = Column(String)
    # order_status = Column(ENUM('pending', 'ongoing', 'done', name='extra_order_statuses'))
...
class StoreOffer(Model):
    ...
    sub_order_id = Column(UUIDType(binary=False), ForeignKey('sub_order.id'), nullable=False)
